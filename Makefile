...
.PHONY: help setup setup-node setup-ui setup-browser-ui setup-midi clean smoke-test run-sse run-stdio run-daw run-node run-node-ui run-node-stdio run-node-stdio-ui run-node-stdio-session run-browser-ui run-browser-stdio run-browser-static client-sse client-stdio client-daw emit-wrapped-dsp rt-compile rt-start rt-get-status rt-get-params rt-get-param rt-get-param-values rt-get-dsp-json rt-get-audio-metrics rt-get-audio-metrics-scope rt-get-audio-metrics-spectrum rt-get-audio-metrics-full rt-get-audio-metrics-full-per-channel rt-set-param rt-save-wasm rt-load-wasm rt-stop rt-destroy rt-midi-list rt-midi-select rt-ws-metrics stop-node test-node-api test-browser-api
...
help:
	...
	@printf "  rt-start      Start audio on real-time server\n"
	...
	@printf "  rt-destroy    Destroy real-time DSP and clear state\n"
	@printf "\nVars:\n"
	@printf "  MCP_HOST=%s\n" "$(MCP_HOST)"
	@printf "  MCP_PORT=%s\n" "$(MCP_PORT)"
...
rt-start:
	$(PYTHON) sse_client_example.py --url http://$(MCP_HOST):$(MCP_PORT)/sse --tool start
...
rt-destroy:
	$(PYTHON) sse_client_example.py --url http://$(MCP_HOST):$(MCP_PORT)/sse --tool destroy
